...
exports.addAudio = function(req,res){
  ...
  form.parse(req, function (err, fields, files) {
    ...
    var newpath = path.join(__dirname, '../Music/Music'+files.file.name);   
    ...
    NodeID3.read(oldpath, function(err, tags) {
      ...
      db.all('SELECT name FROM Music',[],function(error,rows,fields){
        var nameL=[]
        rows.forEach((row)=>{
          nameL.push(row.name)
        });
      ...
      res.send("<script type='text/javascript'> alert('No file selected');location.href = 'http://localhost:8012/addMedia'</script>");
        }else if(nameL.includes(tags.title)){
          res.send("<script type='text/javascript'> alert('Already exists');location.href = 'http://localhost:8012/addMedia'</script>");
        }else{
          db.run('INSERT INTO Music(name,artist_name,image,path) VALUES(?,?,?,?)',metadata, function (error, results, fields) {
            if (error) {
              console.log("error ocurred",error);
...
exports.getAllAudio = function(req,res){
  db.all('SELECT * FROM Music ',[], function (error, rows) {
  if (error) {
    console.log("error ocurred",error);
    res.send({
      "code":400,
