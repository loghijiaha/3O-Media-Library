...
exports.addEbook = function(req,res){
  ...
  form.parse(req, function (err, fields, files) {
    ...
    var newpath = path.join(__dirname, '../Ebook/Ebook'+files.file.name);   
    ...
    NodeID3.read(oldpath, function(err, tags) {
      ...
      res.send("<script type='text/javascript'> alert('No file selected');location.href = 'http://localhost:8012/addMedia'</script>");
          ...
          db.run('INSERT INTO Ebook(name,artist_name,image,path) VALUES(?,?,?,?)',metadata, function (error, results, fields) {
            if (error) {
              console.log("error ocurred",error);
...
exports.getAllEbook = function(req,res){
  db.all('SELECT * FROM Ebook ',[], function (error, rows) {
  if (error) {
    console.log("error ocurred",error);
    res.send({
      "code":400,
